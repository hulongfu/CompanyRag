...
/**
 * 向量检索节点：执行单路向量检索并写入图状态。
 *
 * <p>读取状态中的查询参数，调用 VectorRetriever.retrieve 获取结果并写入 VECTOR_CHUNKS 键。
 * 当单路检索失败时，按原流水线的容错语义降级为空列表而非向上抛异常。</p>
 */
@Slf4j
public class VectorRetrieveNode implements AsyncNodeAction {

    /** 向量检索的固定 topK，与原 MultiRetrieveServiceImpl 保持一致。 */
    ...
    public CompletableFuture<Map<String, Object>> apply(OverAllState state) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                out.put(WorkflowKeys.VECTOR_CHUNKS, chunks);
            } catch (Exception e) {
                // 单路检索失败时降级为空列表，保持对外行为稳定
                log.warn("向量检索节点失败，降级为空结果 | error={}", e.getMessage());
                out.put(WorkflowKeys.VECTOR_CHUNKS, Collections.emptyList());
            } finally {
