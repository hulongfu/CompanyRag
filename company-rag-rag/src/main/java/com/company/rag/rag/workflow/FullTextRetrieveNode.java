...
/**
 * 全文检索节点：执行单路全文检索并写入图状态。
 *
 * <p>读取状态中的查询参数，调用 FullTextRetriever.retrieve 获取结果并写入 KEY_FULLTEXT 键。
 * 当单路检索失败时，按原流水线的容错语义降级为空列表而非向上抛异常。</p>
 */
@Slf4j
public class FullTextRetrieveNode implements AsyncNodeAction {

    /** 全文检索的固定 topK，与原 MultiRetrieveServiceImpl 保持一致。 */
    ...
    public CompletableFuture<Map<String, Object>> apply(OverAllState state) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                out.put(WorkflowKeys.FULLTEXT_CHUNKS, chunks);
            } catch (Exception e) {
                // 单路检索失败时降级为空列表，保持对外行为稳定
                log.warn("全文检索节点失败，降级为空结果 | error={}", e.getMessage());
                out.put(WorkflowKeys.FULLTEXT_CHUNKS, Collections.emptyList());
            } finally {
