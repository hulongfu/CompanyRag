...
AuthController 是认证控制器，提供用户登录、令牌刷新和登出功能，支持基于 JWT 的无状态认证和多租户访问。

**主要功能**：
- **用户登录**：验证用户名密码，生成 Access Token 和 Refresh Token
- **令牌刷新**：使用 Refresh Token 获取新的 Access Token
- **用户登出**：清除安全上下文和会话，记录审计日志

**技术特性**：
- 基于 Spring Boot 3.4 + Spring Security 6
- JWT (JJWT) 令牌生成与验证
- 双令牌机制：Access Token + Refresh Token 分离
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| refreshToken | String | 是 | Refresh Token（JWT 格式） |

**来源**: [AuthController.java](../../../company-rag-web/src/main/java/com/company/rag/web/controller/AuthController.java)(L205-L209)
...
**来源**: [AuthController.java](../../../company-rag-web/src/main/java/com/company/rag/web/controller/AuthController.java)(L31-L42)
